...
module.exports = function(projectdir, appdir, appext, config, built) {
  ...
  glob("" + appdir + "/models/**/*" + appext, {
    ...
    var file, fn, i, model, name, names, schema, _i, _len, _results;
    if (err) {
      console.log(err);
      process.exit(0);
    }
    ...
    for (i in files) {
      ...
      _results.push(mean.register(names[i] + 'Model', fn(mean.get(names[i] + 'Model'))));
    }
    return _results;
  });
  ...
  glob("" + appdir + "/controllers/**/*" + appext, {
    ...
    var controller, file, fn, i, name, names, _i, _len, _results;
    if (err) {
      console.log(err);
      process.exit(0);
    }
    ...
    for (i in files) {
      ...
      _results.push(mean.register(names[i] + 'Ctrl', fn(mean.get(names[i] + 'Ctrl'))));
    }
    return _results;
  });
  plugins = {};
