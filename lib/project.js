...
var dependable, ect, express, fs, glob, mongoose, path, plugin, postrender, utils, _;
...
module.exports = function(projectdir, appdir, appext, config, built) {
  var app, appname, assetfile, assets, chainware, connection, controllers, defaults, dir, engine, globdir, k, locals, mean, models, n, obj, overrides, p, pkg, plugins, publicdir, regex, relappdir, relprojectdir, router, store, vhost, vhosts, views, _i, _j, _len, _len1, _ref;
  ...
  glob("" + appdir + "/models/**/*.{js,coffee}", {
    ...
    for (_i = 0, _len = files.length; _i < _len; _i++) {
      file = files[_i];
      _results.push(utils.aggregate(models, file, dir, mean.resolve(require(file))));
    }
    return _results;
  });
  ...
  glob("" + appdir + "/controllers/**/*.{js,coffee}", {
    ...
    for (_i = 0, _len = files.length; _i < _len; _i++) {
      file = files[_i];
      _results.push(utils.aggregate(controllers, file, dir, mean.resolve(require(file))));
    }
    return _results;
  });
  ...
  locals[appname].view = function(str) {
