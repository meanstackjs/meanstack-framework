...
module.exports = function(projectdir, appdir, appext, config, built) {
  var app, appname, assetfile, assets, chainware, connection, constants, controllers, defaults, dir, engine, fn, globdir, k, key, locals, mean, models, mount, n, obj, overrides, p, pkg, plugin, plugins, publicdir, r, regex, relappdir, relprojectdir, route, router, routes, schemas, store, value, variables, vhost, vhosts, views, _i, _j, _k, _len, _len1, _len2, _ref;
  ...
  glob("" + dir + "/**/*" + appext, {
    ...
    for (_i = 0, _len = files.length; _i < _len; _i++) {
      ...
      if (excluded === ("server" + appext) || excluded === ("chainware" + appext)) {
        continue;
      }
      mdl = require(file);
      if (mdl.schema != null) {
        _.forOwn(mdl.schema, function(prop, key) {
          mean.register(key, prop);
          return schemas[key] = prop;
        });
      }
      if (mdl.model != null) {
        _.forOwn(mdl.model, function(prop, key) {
          mean.register(key, prop);
          return models[key] = prop;
        });
      }
      if (mdl.controller != null) {
        _.forOwn(mdl.controller, function(prop, key) {
          mean.register(key, prop);
          return controllers[key] = prop;
        });
      }
      if (mdl.constant != null) {
        _.forOwn(mdl.constant, function(prop, key) {
          mean.register(key, prop);
          return constants[key] = prop;
        });
      }
      if (mdl.variable != null) {
        _.forOwn(mdl.variable, function(prop, key) {
          mean.register(key, prop);
          return variables[key] = prop;
        });
      }
      if (mdl.route != null) {
        _results.push(routes.push(mdl.route));
      } else {
  ...
  locals[appname].namespace = function(str) {
  ...
  for (k in mean.get('$plugins')) {
    ...
    locals[k].namespace = function(str) {
